...
class SocialAccountModel(BaseModel):
    ...
    scope_granted: Optional[List[str]] = Field(None, description="부여된 권한 스코프 목록")
    ...
    def validate_provider(cls, v: str) -> str:
        ...
        if v.lower() not in allowed_providers:
            raise ValueError(f"지원되지 않는 OAuth 제공자입니다: {v}. 지원 제공자: {allowed_providers}")
        return v.lower()
    ...
    def validate_provider_user_id(cls, v: str) -> str:
        ...
        if not v.replace('-', '').replace('_', '').isalnum():
            raise ValueError("제공자 사용자 ID는 영숫자, 하이픈, 언더스코어만 포함할 수 있습니다")
