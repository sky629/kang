...
class DocumentResponse(BaseModel):
    ...
    created_at: datetime
    updated_at: datetime
    chunks: Optional[List[DocumentChunk]] = Field(default=None, description="문서 청크들")
...
class RAGQueryResponse(BaseModel):
    ...
    sources: Optional[List[DocumentResponse]] = Field(default=None, description="참조 문서들")
...
class RAGQuery(BaseModel):
    ...
    answer: str = Field(..., description="생성된 답변")
    context_documents: Optional[List[str]] = Field(default=None, description="참조 문서 컨텍스트")
...
class RAGQueryResponse(BaseModel):
    ...
    confidence_score: Optional[float] = Field(None, ge=0.0, le=1.0, description="답변 신뢰도")
    search_time_ms: Optional[int] = Field(None, description="검색 소요 시간 (ms)")
    generation_time_ms: Optional[int] = Field(None, description="답변 생성 소요 시간 (ms)")